...
- Build pipeline checks lockfile existence before compiling
- `owl build` / `owl run` without lockfile → generates lockfile automatically
- Phase 3 still missing: sync checking (lockfile vs owl.toml mismatch detection)
...
- **`code/lockfile/mod.mire`**: new module handling lockfile generation
